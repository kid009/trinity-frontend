  ...
  <div>
    <div class="grid-content clearfix">
      <div
        ...
        v-for="items in knowhow"
        v-bind:key="items.bussiness_knowhow_id"
      >
        <div class="hotel-item style-10 bg-white">
          <div class="table-view">
            <a @click="on_click(1, 2, items.bussiness_knowhow_id)">
              <div class="radius-top">
                <img :src="items.bussiness_knowhow_image_cover_link" alt="" />
              </div>
              <div class="title">
                <h4>
                  <b>{{ items.bussiness_knowhow_name.substring(0, 50) }}</b>
                </h4>
                <!-- <button
...
export default {
  name: "BusinessLearnningKnowhowIndexGrid",
  ...
  setup() {
    ...
    const getData = async () => {
      if (id == 0 && group == 0) {
        const response = await axios.get(
          `${BASE_API_URL}/businesstourism/BusinessKnowhow?page=` +
            page +
            "&group=0&id=0"
        );
    ...
    function on_click(page, group, id) {
      window.location =
        "businesslearnningknowhowdetailpage?page=" +
        page +
        "&group=" +
        group +
        "&id=" +
    ...
    return { knowhow, on_click, id, page, group };
  },
};
</script>
