<template>
  <div class="container box" v-show="business[0] != null">
  ...
  setup() {
    ...
    const getData = async () => {
      const responsebusiness = await axios.get(
        `${BASE_API_URL}/accommodation/business?page=1&group=0&id=` + id
      );
    ...
    return { business, on_click, page, group, id };
  },
};
</script>
