      ...
      >
        <div class="hotel-item style-10 bg-white">
          <div class="table-view">
            <a @click="on_click(page, group, items.activity_id)">
              <div class="radius-top">
                <img :src="items.activity_image_cover_link" alt="" />
              </div>
              <div class="title">
                <h4>
                  <b>{{ items.activity_name.substring(0, 50) }}</b>
                </h4>
                ...
                <main>
                  ...
                  <article>
                    <a class="text-end">{{ items.activity_group_name }}</a>
                  </article>
                </main>
              </div>
            </a>
...
export default {
  ...
  setup() {
    ...
    const getData = async () => {
      if (id == 0 && group == 0) {
        const response = await axios.get(
          `${BASE_API_URL}/tourism-route/activitAll?page=` +
            page +
            "&group=0&id=0"
        );
    ...
    function on_click(page, group, id) {
      window.location =
        "ActivityDetailAll?page=" + page + "&group=" + group + "&id=" + id;
    }
    ...
    return { activity, on_click, id, page, group };
  },
};
</script>
