    ...
    id = id[3];
      ...
      const getData = async () => {
        ...
        const response = await axios.get(
          `${BASE_API_URL}/tourism-route/activitAll?page=` + page +'&group=0&id=0'
        );
        pages.value = response.data.quantity_page;
      } else if(group != 0 && id == 0){

        const response = await axios.get(
          `${BASE_API_URL}/tourism-route/activitAll?page= `+ page +'&group=' + group +'&id=0'
        );
        pages.value = response.data.quantity_page;
        ...
        const response = await axios.get(
          `${BASE_API_URL}/tourism-route/activitAll?page=`+ page + '&group=0&id=' + id
        );
        pages.value = response.data.quantity_page;
      }
